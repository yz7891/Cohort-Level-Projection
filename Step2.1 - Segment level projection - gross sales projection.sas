...
/* version 1
%let reg_2y=reg.sales_reg_20170503;
%let reg_5y=reg.sales_5yr_reg_20170208;
%let reg_5y_cp_dt=reg.proxy_5yr_curves_cp_dt_20170208;
%let reg_5y_ic_cs=reg.proxy_5yr_curves_ic_cs_20170208;
*/
/* version 2, modified at 2017-07-20*/
%let reg_2y=reg.sales_reg_20170702;
%let reg_5y=reg.sales_5yr_reg_20170702;
%let reg_5y_cp_dt=reg.proxy_5yr_curves_cp_dt_20170702;
%let reg_5y_ic_cs=reg.proxy_5yr_curves_ic_cs_20170702;
...
/*Backup actual data*/
data rdata.actual_data_&last_version.;
set rdata.actual_data_all;
run;

proc sort data = rdata.actual_data_all out=actual_data_all nodupkey;
...
data fcsales1(drop=startavgsales sales_amt ship_hand_amt);
format m 3.
model $4.;
set fcsales;
by &grouplevel. enr_week;
...
if first.enr_week then do;
    cumsales = sum(0,sales_amt,ship_hand_amt);
    ...
    cumsales = cumsales + sum(0,sales_amt,ship_hand_amt);
    if cumsales = 0 then cumsales = 0.01;
    avgsales = cumsales/starts;
...
proc append base=rdata.actual_data_all data=new_actual_&version._&output. force;
run;
...
proc sort data = rdata.sales_audit_&last_version._v3(where=(type='% difference')) out=audit0;
by model enr_week;
run;
...
set audit_rollup_0(in=a where=(model='s7'))
audit_rollup_0(in=b where=(model='s41'))
audit_rollup_0(in=c where=(model='s41'))
audit_rollup_0(in=d where=(model='s4'))
audit_rollup_0(in=e where=(model='s4'))
audit_rollup_0(in=f where=(model='s4'))
audit_rollup_0(in=g where=(model='s55'))
audit_rollup_0(in=h where=(model='s7'))
audit_rollup_0(in=i where=(model='s55'))
audit_rollup_0(in=j where=(model='s10'))
audit_rollup_0(in=k where=(model='s5'))
audit_rollup_0(in=l where=(model='s5'))
...
data rdata.fc_sale_&version.;
set fc_sale_s1 - fc_sale_s84;
run;

data rdata.fc_rate_&version.;
set fc_rate_2_s1 - fc_rate_2_s84;
run;

data rdata.forcast_&version.;
set forcast_s1 - forcast_s84;
run;

data rdata.actual_&version.;
set actual_s1 - actual_s84;
run;

proc sort data = rdata.forcast_&last_version. out=forcast_&last_version. nodupkey;
by model enr_week;
run;

proc sort data = rdata.actual_&version. out=actual_&version. nodupkey;
by model enr_week;
run;

data rdata.compare_&version.;
...
data rdata.new_actual_&version.;
set new_actual_&version._s1 - new_actual_&version._s84;
run;

/*Append New data*/
data rdata.fc_sales_all_&last_version.;
set rdata.fc_sales_all;
run;

data rdata.fc_sales_all;
set rdata.fc_sales_all(in=a where=(update_flag ne &version.))
rdata.fc_sale_&version.(in=b drop=_NAME_ rename=(age104=adj_age104 org_age104=age104));
if b then do;
	update_flag = &version.;
...
%macro outputresult;
/*Sales detail*/
data fc_sale_&version.;
set rdata.fc_sale_&version.;
format modelno 3.
...
/*Rate detail*/
data fc_rate_&version.;
set rdata.fc_rate_&version.;
format modelno 3.
week_ending MMDDYYS10.
%do i=1 %to 260;
age&i. 10.4
...
proc sort data = rdata.compare_&version. out=compare_&version.;
by model;
run;

data compare_&version.2;
merge compare_&version.(in=a)
modellist(in=b);
by model;
if a;
format modelno 3.
...
/*Export file*/
proc sql;
	create table fc_sale_&version._output as
	...
	validation_error
	from fc_sale_&version.
	where productline_cd not in ('PA','DT')
	order by modelno,enr_week1;
	
	create table fc_rate_&version._output as
	select model,productline_cd,venuegroup,supply_size,enr_week1 as enr_week,week_ending,starts,enr_week_no as age_in_weeks,
	%do i=1 %to 103; age&i., %end;
	age104 as 'age104(adjusted)'n,
	%do j=105 %to 260; age&j., %end;
	org_age104 as 'age104(original)'n,
	validation_error
	from fc_rate_&version.
	where productline_cd not in ('PA','DT')
	order by modelno,enr_week1;
	
	create table compare_&version._output as
...
x "del E:\work\br\process\Curve\Report\fc_gs_sale_&version..xlsx";
x "del E:\work\br\process\Curve\Report\fc_gs_rate_&version..xlsx"; 
x "del E:\work\br\process\Curve\Report\gs_validation_details_&version..xlsx";

PROC EXPORT DATA=fc_sale_&version._output
OUTFILE="E:\work\br\process\Curve\Report\fc_gs_sale_&version..xlsx"
DBMS=XLSX
REPLACE;
SHEET='fc_gs_sale';
RUN;

PROC EXPORT DATA=fc_rate_&version._output
OUTFILE="E:\work\br\process\Curve\Report\fc_gs_rate_&version..xlsx"
DBMS=XLSX
REPLACE;
SHEET='fc_gs_rate';
RUN;

PROC EXPORT DATA=compare_&version._output
OUTFILE="E:\work\br\process\Curve\Report\gs_validation_details_&version..xlsx"
DBMS=XLSX
REPLACE;
