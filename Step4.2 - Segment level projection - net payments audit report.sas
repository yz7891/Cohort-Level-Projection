...
proc sort data = rdata.fc_wo_sales_all out=fc nodupkey;
...
proc sort data = rdata.wo_actual_data_all out=actual_data_2yr;
by model productline_cd venuegroup supply_size enr_week;
run;
...
proc sort nodupkey data = reg.net_write_reg_20170702(keep=model m intercept week2 - week104 rename=(m=week)) out=reg;
by model week;
run;
...
proc sort data = rdata.fc_wo_sales_all out=fc_data_2yr nodupkey;
by model enr_week update_date;
run;
...
proc sort data = combine_data2 out=rdata.net_write_audit_&version._v3(drop=nnobs rename=(_NAME_ = type));
...
data net_write_audit_&version._v4;
set rdata.net_write_audit_&version._v3;
where type = '% difference';
run;

proc sort data = net_write_audit_&version._v4;
by model enr_week;
run;

/*Get starts information*/
proc sort data = rdata.fc_wo_sale_&version. out=fc_starts(keep=model enr_week starts) nodupkey;
by model enr_week;
run;

data net_write_audit_&version._v4;
merge net_write_audit_&version._v4(in=a)
fc_starts(in=b);
by model enr_week;
if a;
run;
...
%macro loopavg;
proc sql;
	create table net_write_audit_&version._v4a as
	select model %do i=9 %to 103; ,(sum(starts * age&i.) / sum(starts)) as age&i %end;
	from net_write_audit_&version._v4
	group by 1;
quit;
%mend;
...
data net_write_audit_&version._v4b;
set net_write_audit_&version._v4a;
nnobs = substr(model,2,3) * 1;
run;

proc sort data = net_write_audit_&version._v4b out=rdata.net_write_audit_avg_&version.(drop=nnobs);
by nnobs;
run;
...
/*Audit detail*/
proc sort data = rdata.net_write_audit_&version._v3 out=net_write_audit_&version._v3;
by model;
run;

data net_write_audit_&version._v4;
merge net_write_audit_&version._v3(in=a)
modellist(in=b);
by model;
if a;
format modelno 3.
...
/*Audit summary*/
proc sort data = rdata.net_write_audit_avg_&version. out=net_write_audit_avg_&version.;
by model;
run;

data net_write_audit_avg_&version.2;
merge net_write_audit_avg_&version.(in=a)
modellist(in=b);
by model;
if a;
format modelno 3.;
...
/*Export file*/
proc sql;
	create table net_write_audit_&version._output as
	...
	create table np_audit_avg_&version._output as
	select model,description
	%do i=9 %to 103; ,age&i. %end;
	from net_write_audit_avg_&version.2
	where model not in ('w7','w8','w9','w10','w11','w12','w13','w14','w15','w16','w17','w18','w19','w20','w21','w34','w35','w36','w37')
	order by modelno;
quit;
%mend;
...
x "del E:\work\br\process\Curve\Report\np_audit_&version..xlsx";
x "del E:\work\br\process\Curve\Report\np_audit_avg_&version..xlsx"; 

PROC EXPORT DATA=net_write_audit_&version._output
...
PROC EXPORT DATA=np_audit_avg_&version._output
OUTFILE="E:\work\br\process\Curve\Report\np_audit_avg_&version..xlsx"
DBMS=XLSX
REPLACE;
SHEET='np_audit_avg';
