...
proc sort data = rdata.fc_sales_all out=fc nodupkey;
...
proc sort data = rdata.actual_data_all out=actual_data_2yr;
by model productline_cd venuegroup supply_size enr_week;
run;
...
proc sort nodupkey data = reg.sales_reg_20170702(keep=model m intercept week2 - week104 rename=(m=week)) out=reg;
by model week;
run;
...
proc sort data = rdata.fc_sales_all out=fc_data_2yr nodupkey;
by model enr_week update_date;
run;
...
proc sort data = combine_data2 out=rdata.sales_audit_&version._v3(drop=nnobs rename=(_NAME_ = type));
...
data sales_audit_&version._v4;
set rdata.sales_audit_&version._v3;
where type = '% difference';
run;

proc sort data = sales_audit_&version._v4;
by model enr_week;
run;

/*Get starts information*/
proc sort data = rdata.fc_sale_&version. out=fc_starts(keep=model enr_week starts) nodupkey;
by model enr_week;
run;

data sales_audit_&version._v4;
merge sales_audit_&version._v4(in=a)
fc_starts(in=b);
by model enr_week;
if a;
run;
...
%macro loopavg;
proc sql;
	create table sales_audit_&version._v4a as
	select model %do i=9 %to 103; ,(sum(starts * age&i.) / sum(starts))as age&i %end;
	from sales_audit_&version._v4
	group by 1;
quit;
%mend;
...
data sales_audit_&version._v4b;
set sales_audit_&version._v4a;
nnobs = substr(model,2,3) * 1;
run;

proc sort data = sales_audit_&version._v4b out=rdata.sales_audit_avg_&version.(drop=nnobs);
by nnobs;
run;
...
/*Audit detail*/
proc sort data = rdata.sales_audit_&version._v3 out=sales_audit_&version._v3;
by model;
run;

data sales_audit_&version._v4;
merge sales_audit_&version._v3(in=a)
modellist(in=b);
by model;
if a;
format modelno 3.
...
/*Audit summary*/
proc sort data = rdata.sales_audit_avg_&version. out=sales_audit_avg_&version.;
by model;
run;

data sales_audit_avg_&version.2;
merge sales_audit_avg_&version.(in=a)
modellist(in=b);
by model;
if a;
format modelno 3.;
...
/*Export file*/
proc sql;
	create table sales_audit_&version._output as
	...
	create table sales_audit_avg_&version._output as
	select model,description
	%do i=9 %to 103; ,age&i. %end;
	from sales_audit_avg_&version.2
	where model not in ('s17','s18','s19','s20','s21','s22','s23','s24','s25','s26','s27','s28','s29','s30','s31','s32','s33','s34','s35','s59','s60','s61','s62')
	order by modelno;
quit;
%mend;
...
x "del E:\work\br\process\Curve\Report\gs_audit_&version..xlsx";
x "del E:\work\br\process\Curve\Report\gs_audit_avg_&version..xlsx"; 

PROC EXPORT DATA=sales_audit_&version._output
OUTFILE="E:\work\br\process\Curve\Report\gs_audit_&version..xlsx"
DBMS=XLSX
REPLACE;
SHEET='gs_audit';
RUN;

PROC EXPORT DATA=sales_audit_avg_&version._output
OUTFILE="E:\work\br\process\Curve\Report\gs_audit_avg_&version..xlsx"
DBMS=XLSX
REPLACE;
SHEET='gs_audit_avg';
RUN;
